...
'new2','new1','new39','new41','new43','new45'],['new40','new42','new44'],2,2);abnormalStatusChange()">
              ...
              <div>
                <el-row class="block-title">
                  <el-form-item label="三、本次检查是否存在胸腔积液或胸膜异常（胸膜增厚或斑块、胸膜钙化、胸膜肿物）和胸壁异常？" 
...
export default {
  ...
  data() {
    ...
    var validate_lessTenCount = (rule, value, callback) => {
      if (value < 1 || value > 10 || !/^\d{1,2}$/.test(value)) {
    ...
    var validate_greaterTenCount = (rule, value, callback) => {
      if (value < 11 || !/^\d{1,2}$/.test(value)) {
        this.countState = false;
        callback(new Error("数值（11-99）且为正整数"));
      } else {
        callback();
        this.countState = true;
      }
    ...
    return {
      ...
      formRules: {
        ...
        new9: [{ required: true, message: "必填", trigger: "change" }],
        new10: [
          { required: true, message: "必填", trigger: "change" },
        ...
        new16: [{ required: true, message: "必填", trigger: "blur" }],
        new17: [
          { required: true, message: "必填", trigger: "change" },
          { validator: validate_new17, trigger: "change" }
        ],
        new18: [{ validator: validate_new18, trigger: "change" }],
        new19: [{ required: true, message: "必填", trigger: "blur" }],
  ...
  methods: {
    ...
    addTableTubercleList() {
      ...
      if(this.form.item8_3 == "1" &&
        ...
        // 结节大于10最多只有10个
      if (this.form.item8_3 == "2" && this.form.greaterTenCount > 10) {
        let n=this.form.lessTenCount-this.form.ctTubercleInfoPOList.length
        for (let i = this.form.ctTubercleInfoPOList.length; i < 10; i++) {
          this.form.ctTubercleInfoPOList.push({
            tubercleCode: i + 1,
            numName: null, //结节编号
            serialNumber: "", //序列编号
