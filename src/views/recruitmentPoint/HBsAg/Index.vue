<template>
  <div>
    <!--表单查询-->
    <search-com :labelWidth="'100px'" :searchData="searchData" :formData="qc" ref="searchHBsAg"
                    @searchChange="searchHandler"></search-com>
    <div class="table-con-appointment">
      <cancer-widget>
        <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
          <el-tab-pane label="全部" name="1">
            <HBsAgTable :tableData="tableData" @refreshList="query"></HBsAgTable>
          </el-tab-pane>
...
<script>
import HBsAgTable from "./HBsAgTable"
import { HBsAg } from '@/views/recruitmentPoint/HBsAg/searchConfig'
  export default {
    data () {
      return {
        searchData: HBsAg,
    ...
    methods: {
      ...
      async query(){
        ...
        const  {data: {status, result}} = await this.$axios_http({
          url: "/base/measurement/find/hBsAgList",
      ...
      pageSizeChange(pageSize) {
        this.qc = this.getFormData('searchHBsAg');
