<template>
    <div class="screeningCollect-container">
    <!--查询表单-->
    <search-com :labelWidth="'100px'" :searchData="searchData" ref="screeningCollect" :formData="qc"
...
export default {
    ...
    methods: {
        query(){
          this.$axios_http({
              url: "/base/zm/booking/bookedPersonForSendReport",
      ...
      reportView(row,type){
        if(type == 'ct'){
          ...
          this.$router.push({
            ...
            //跳转tab切换页面 
            this.$router.push({
              path:'/form/screeningCollectView',
              query:{
                  surverId:row.surverId,
                  personId:row.personId,
