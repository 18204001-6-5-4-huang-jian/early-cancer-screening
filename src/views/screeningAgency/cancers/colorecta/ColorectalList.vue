<template>
  <div>
     <!--查询表单-->
    <search-com :labelWidth="'100px'" :searchData="searchData" ref="searchColo" :formData="qc"
    ...
    <div class="table-con-appointment">
      <cancer-widget isBorder>
              ...
              >
                <template slot-scope="scope">
                  <el-button 
                    type="text" 
                    size="small"
                    v-if="scope.row.resultInputStatus==2" 
                  ...
                  <!-- <router-link :to="{path:'/form/enteroscopyentry',query:{
                    ...
                    signedDate:scope.row.signedDate}}" v-if="scope.row.resultInputStatus==3">
        ...
        <!-- <el-tab-pane :label="'筛查结果待提交('+num.unSubmit+')'" name="3">
          <ColorectaTable :tableData="tableData" @refreshList="query"></ColorectaTable>
        </el-tab-pane> -->
              ...
              >
                <template slot-scope="scope">
                  ...
                  <el-button 
                    type="text" 
                    size="small"
                    v-if="scope.row.resultInputStatus==1 && scope.row.pathologyEnterStatus==1 && scope.row.pathologyReportEnterStatus!=1"
                    @click="generatePathologyReport(scope.row)">生成病理报告</el-button>
                  <el-button 
                    type="text" 
                    size="small"
                    v-if="scope.row.pathologyReportEnterStatus==1"
...
import { COLORECTA } from "./searchConfig";
import { COLORECTATB } from "./tablePropConfig";
export default {
  data() {
    return {
      searchData: COLORECTA,
      propsData: COLORECTATB,
      activeName: "1",
      inputStatus: DICTIONARY.inputStatus, //录入状态
      colorectaResult: DICTIONARY.colorectaResult, //筛查结果
  ...
  methods: {
    ...
    watchReport(row) {
      this.saveSearchParams();
      this.$router.push({
        path: "/form/enteroscopyreportlook",
        query: {
          screeningId: row.screeningId,
          id: row.id,
          surverId: row.surverId,
    ...
    watchPathology(row) {
      ...
      this.$router.push({
        ...
        query: {
          ...
          pathologyReportEnterStatus: row.pathologyReportEnterStatus
        }
      });
    },
    // 生成病理报告
    ...
    query() {
      ...
      let pathologyEnterStatus = null;
      if (this.activeName == "1") {
        // 待录入
        inputStatus = 2;
      } else if (this.activeName == "2") {
        pathologyEnterStatus = 2;
      } else if (this.activeName == "3") {
        inputStatus = 3;
      } else if (this.activeName == "4") {
        inputStatus = 1;
        pathologyEnterStatus = 1;
      }
      delete this.qc.checkDate;
      this.$axios_http({
        url: "/base/cj/find/list/condition",
        data: Object.assign({}, this.qc, {
          inputStatus: inputStatus,
          pathologyEnterStatus: pathologyEnterStatus,
    ...
    pageSizeChange(pageSize) {
      this.qc = this.getFormData("searchColo");
