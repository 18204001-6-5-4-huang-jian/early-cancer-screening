<template>
    <div>
        <el-dialog
            ...
            <el-row v-for="(item,index) in detailList" :key="index" v-if="detailList.length"
                ...
                <el-col :span="12">
                    ...
                    <frozen-box ref="previewFrozen" :usedData="item.locationValues"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 查看详情
            async sampleDetailData(val) {
                let {data: {status, result}} = await this.$axios_http({
                    url: `/base/biological/get/biologicalSampleInfo/${val}`,
                    method: 'post'
                })
                if (status === 'SUCCESS') {
