...
export default {
  methods: {
    ...
    async checkSampleIdData (val) {
      ...
      if (status === 'SUCCESS') {
        if (result.person.flag === 1) {
          // 跳转到录入
          this.listName=result.person.name
          this.itemsUnderIt = result.itemsUnderIt
          this.$refs['writeDialog'].writeData(this.mainSampleId, result.surverId, result.person.id, () => {
            this.sampleDialogVisible = true
          })
          this.$refs['filterWriteDialog'].closeDialog()
        } else if (result.person.flag === 0) {
          // 显示身份证号
          this.isShowIdCard = 0
        }
      }
    ...
    async submitData (val) {
      ...
      if (status === 'SUCCESS') {
        ...
        this.itemsUnderIt = result.itemsUnderIt
        this.$refs['writeDialog'].writeData(this.mainSampleId, result.surverId, result.personId, () => {
          this.sampleDialogVisible = true
        })
        this.$refs['filterWriteDialog'].closeDialog()
      }
