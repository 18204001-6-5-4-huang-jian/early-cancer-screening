<template>
  ...
  <div>
    ...
    <cancer-widget>
      <p class="cancer-title-blue" style="margin-top: -40px;">任务列表</p>
      <template slot="sub-button">
        <el-button type="primary" size="small" @click="addHandler()">新增任务</el-button>
      </template>
      <el-table
        ...
        <el-table-column
          label="癌种">
          <template slot-scope="scope">
            {{scope.row.cancerType | cancerTypeFilters}}
          </template>
        </el-table-column>
        <el-table-column
          prop="taskNum"
...
<script>
  import service from '@/views/gj/task/task-service'
  ...
  export default {
    ...
    methods: {
      ...
      // 添加
      addHandler () {
        this.jumpToPage('/gj/task/addtask')
